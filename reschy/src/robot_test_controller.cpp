...
// 2015.10.4  | Ground zero | for testing ros serial.
//
//
#include <ros/ros.h>
...
main (int argc, char** argv)
{
	...
	// publishder
  	pubTest = nh.advertise<std_msgs::String> ("reschy/status/run/ladder", 1);	// "toggle_led", 1); // reschy/test/control", 1);
	
	// timer with 0.2 second (5 Hz) for running each steps of mission considering sensor data sync 
	ros::Timer timerRun = nh.createTimer(ros::Duration(1.0), callbackTimerRun);
